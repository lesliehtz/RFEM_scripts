...
LoadCase.prototype.StaticAnalysis = function (no, name, staticAnalysisSettingsNo, ActionCategory, selfWeighParams, stabilityAnalysisSettingsNo, comment, params) {
  ...
  if (typeof ActionCategory !== "undefined") {
...
/**
 * Creates Time history analysis | Time diagram load case
 * @param {number} no                                     Index of load case, can be undefined
 * @param {String} name                                   Name
 * @param {Number} time_history_analysis_settings         Time history analysis setting number
 * @param {Number} import_masses_from                     Import masses only from load case/combination number
 * @param {Number} time_history_time_diagram              Time diagram number
 * @param {Number} time_history_time_diagram_multiplier   Multiplier, can be undefined (is not set, 1.0 as default) 
 * @param {Number} time_history_time_diagram_time_slip    Time slip, can be undefined (is not set, 0.0 as default)
 * @param {String} comment                                Comment, can be undefined
 * @param {String} params                                 Additional parameters, can be undefined
 * @returns Created load case
 */
LoadCase.prototype.TimeHistoryAnalysis = function (no,
  name,
  time_history_analysis_settings,
  time_history_import_masses_from,
  time_history_time_diagram,
  time_history_time_diagram_multiplier,
  time_history_time_diagram_time_slip,
  comment,
  params) {
  ASSERT(typeof time_history_analysis_settings !== "undefined", "Time history analysis settings number is required");
  ASSERT(typeof time_history_import_masses_from !== "undefined", "Import masses only from load case/combination number is required");
  ASSERT(typeof time_history_time_diagram !== "undefined", "Time diagram number is required");
  this.LoadCase = CreateLoadCase(no, name);
  this.LoadCase.analysis_type = load_cases.ANALYSIS_TYPE_TIME_HISTORY_TIME_DIAGRAM;
  this.LoadCase.time_history_analysis_settings = time_history_analysis_settings;
  this.LoadCase.time_history_import_masses_from = time_history_import_masses_from;
  this.LoadCase.time_history_time_diagram = time_history_time_diagram;
  if (typeof time_history_time_diagram_multiplier !== "undefined") {
    this.LoadCase.time_history_time_diagram_multiplier = time_history_time_diagram_multiplier;
  }
  if (typeof time_history_time_diagram_time_slip !== "undefined") {
    this.LoadCase.time_history_time_diagram_time_slip = time_history_time_diagram_time_slip;
  }
  set_comment_and_parameters(this.LoadCase, comment, params);
  var self = this;
  return self;
...
function CreateLoadCase(no, name) {
  if (typeof no === "undefined") {
    no = load_cases.count() + 1;
  }
  var LoadCase = engine.create_load_case(no);
  ...
  if (LC !== undefined) {
    if (selfWeighParams !== undefined && selfWeighParams.length !== 0) {
      if (selfWeighParams[0] === true && selfWeighParams.length === 4) {
        console.log(selfWeighParams);
        LC.self_weight_active = true;
        LC.self_weight_factor_x = selfWeighParams[1];
        LC.self_weight_factor_y = selfWeighParams[2];
        LC.self_weight_factor_z = selfWeighParams[3];
      }
      else {
