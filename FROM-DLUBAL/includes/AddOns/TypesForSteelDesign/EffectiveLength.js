...
2. No API enum for Determination of Mcr (NTC standard)?
3. There is no support for Modification factor for CSA and NBR standards? Only for AISC?
4. ModificationFactor: NBR standard, user-value cannot be set?
*/
 ...
 * @param {String} standard_of_effective_lengths    Standard (AISC: AISC_360, AISI_S100, CSA: CSA_S16, CSA_S136), can be undefined (if not set, AISC_360 as default)
 */
SteelDesignEffectiveLength.prototype.SetEffectiveLengthsAccToStandard = function (standard_of_effective_lengths) {
    ASSERT(IsCurrentCodeOfStandard("AISC") || IsCurrentCodeOfStandard("CSA"), "Effective length acc. to standard can be set only for AISC or CSA code of standard");
...
SteelDesignEffectiveLength.prototype.SetSegmentsUnrestrainedOneEnd = function (moment_modification_unrestrained_segments_as,
    ...
    if (typeof modification_factor_alpha_unrestrained_segments_as !== "undefined") {
        ASSERT(this.effective_length.moment_modification_unrestrained_segments_as === GetSteelDesignMomentModificationUnrestrainedType("USER_DEFINED"), "Factor alpha can be set only with user-defined moment modification type");
    ...
    if (typeof slenderness_reduction_unrestrained_segments_as !== "undefined") {
        console.log(slenderness_reduction_unrestrained_segments_as);
        this.effective_length.slenderness_reduction_unrestrained_segments_as = EnumValueFromJSHLFTypeName(
            slenderness_reduction_unrestrained_segments_as,
            "slenderness reduction unrestrained segment",
            {
            ...
            steel_effective_lengths.SLENDERNESS_ALPHA_ACC_TO_5611);
    }
};
...
function GetSteelDesignEffectiveLengthAccToStandardType (standard_type) {
    if (IsCurrentCodeOfStandard("AISC")) {
        ...
        ASSERT(IsCurrentCodeOfStandard("CSA"));
        standard_types = {
            "CSA_S16": steel_effective_lengths.STANDARD_OF_EFFECTIVE_LENGTHS_CSA_S16,
            "CSA_S136": steel_effective_lengths.STANDARD_OF_EFFECTIVE_LENGTHS_AISI_S100
        }
    }
...
function GetSteelDesignSlendernessReductionRestrainedType (slenderness_type) {
    const slenderness_types = {
        "ACC_TO_5611": steel_effective_lengths.SLENDERNESS_RESTRAINED_ALPHA_ACC_TO_5611,
        "EIGENVALUE_METHOD": steel_effective_lengths.SLENDERNESS_RESTRAINED_ALPHA_EIGENVALUE_METHOD
    };
