...
var LCSW = new LoadCase().StaticAnalysis(undefined, "Static analysis", SASGeometricallyLinear.Settings.no, "IMPOSED_LOADS_CATEGORY_A_DOMESTIC_RESIDENTIAL_AREAS_QI_A", [true, 1.0, 2.0, 3.0]);
...
DYNAMIC_ANALYSIS.TIME_HISTORY.setActive(true)

var time_diagram_values = [];
for (var i = 0; i < 40; i += 2) {
    time_diagram_values[i] = 0.1 * i;
    time_diagram_values[i + 1] = i;
}
var timeDiagram = new TimeDiagram();
timeDiagram.UserDefined(undefined, time_diagram_values, true, 0.5);
timeDiagram.SetStart("CONSTANT");
timeDiagram.SetEnd("CONSTANT");
var timeDiagram2 = new TimeDiagram();
timeDiagram2.Function(undefined, "t + 0.1 * t^2", 15);

var designSituation = new DesignSituation(undefined, "EQU_PERMANENT_AND_TRANSIENT");
var staticAnalysisSetting = static_analysis_settings.create(static_analysis_settings.lastId() + 1);
var loadCombination = new LoadCombination();
loadCombination.StaticAnalysis(undefined, staticAnalysisSetting.no, designSituation.GetNo(), undefined, {"name": "Load combination for Time history"});
var LCTimeHistory = new LoadCase().TimeHistoryAnalysis(undefined, "Time history analysis", time_history_analysis_settings[1].no, loadCombination.GetNo(), timeDiagram2.GetNo(), 2.0, 0.5);
