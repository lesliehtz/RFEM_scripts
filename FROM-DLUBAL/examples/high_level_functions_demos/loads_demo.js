...
var nodalLoad6 = new NodalLoad();
nodalLoad6.Force(undefined, lc, [3], 1000, "GLOBAL_Z_OR_USER_DEFINED_W");
// Add specific direction with Rotated via 3 angles direction type (1), sequence = Z'Y'X'
// Parameter values: 1, [αZ', αY', αX', "Z'Y'X'"]
nodalLoad6.SpecificDirection(1, [10, 5, 0, "Z'Y'X'"]);
// Force nodal load
var nodalLoad5 = new NodalLoad();
nodalLoad5.Force(undefined, lc, [4], 1800);
// Add shifted display
// Parameter values: [ΔX, ΔY, ΔZ], Δ
nodalLoad5.ShiftedDisplay([1, 0.5, 0], 0.1);

var nodalLoad2 = new NodalLoad();
// Moment nodal load with default "X" load direction
...
if (RFEM) {
	...
	lineLoad.Force(undefined, lc, [6, 8], "CONCENTRATED_2x2", [1000, 1, 0.4, 0.5, false, false, true]);
	...
	lineLoad2.Moment(undefined, lc, [9], "PARABOLIC", [1500, 2000, 1000], "GLOBAL_X_OR_USER_DEFINED_U_TRUE");
...
memberLoad2.Force(undefined, lc, [2], "CONCENTRATED_1", [500, 0.5, true]);
// Force concentrated - varying member load
// Parameter values: [P1, x1, P2, x2 ... Pn, xn]
memberLoad2.Force(undefined, lc, [3], "CONCENTRATED_VARYING", [100, 0.5, 200, 1.5, 1000, 3]);
...
memberLoad2.Force(undefined, lc, [5], "PARABOLIC", [1500, 200, 1600]);
// Force uniform load
memberLoad2.Force(undefined, lc, [17], "UNIFORM", [2500]);
// Add eccentricity
memberLoad2.Eccentricity("CENTER_TOP", 0.01, 0.02, 0.03, 0.04);
...
var memberLoad3 = new MemberLoad(undefined, lc, [6], "Moment parabolic load sets via parameters",
	{ "load_type": member_loads.LOAD_TYPE_MOMENT, "load_distribution": member_loads.LOAD_DISTRIBUTION_PARABOLIC, "magnitude_1": 500, "magnitude_2": 700, "magnitude_3": 1500, "load_direction": member_loads.LOAD_DIRECTION_GLOBAL_X_OR_USER_DEFINED_U_TRUE });
var memberLoad4 = new MemberLoad();
// Moment varying load with global in X on true length
memberLoad4.Moment(undefined, lc, [7], "VARYING", [500, 0.1, 800, 1, 1200, 2, 600, 3], "GLOBAL_X_OR_USER_DEFINED_U_TRUE");
...
memberLoad5.Temperature(undefined, lc, [9], "TRAPEZOIDAL", [300, 2.9, 280, 320, 290, 0.2, false, true], "LOCAL_Z");
// Temperature varying member load
memberLoad5.Temperature(undefined, lc, [10, 11], "VARYING", [300, 330, 0.1, 310, 330, 2, 300, 340, 2.9], "LOCAL_Z");
...
memberLoad6.TemperatureChange(undefined, lc, [12], "PARABOLIC", [60, 10, 50, 20, 100, 10], "LOCAL_Z");

var memberLoad8 = new MemberLoad();
//memberLoad8.PipeContentFull(undefined, lc, [18], 1500, "-ZL");	// Bug? Can't set load direction (it's not allowed)
memberLoad8.PipeContentFull(undefined, lc, [18], 1500);
...
if (RFEM) {
	...
	surfaceLoad.Force(undefined, lc, [6], "RADIAL", ["TWO_POINTS", 1000, 500, 74, 68, [67, 68]]);
	...
	surfaceLoad.Force(undefined, lc, [6], "RADIAL", ["POINT_AND_AXIS", 1000, 500, 74, 68, 0.5, 0.5, 0, "Z"]);
	// Force varying in Z surface load
	// Parameter values: [p1, z1, p2, z2, ... pn, zn]
	surfaceLoad.Force(undefined, lc, [7], "VARYING_IN_Z", [2000, -3, 1000, -2, 1500, -1.5, 1500, -0.5, 500, 0]);

	var surfaceLoad2 = new SurfaceLoad();
	// Temperature linear in X surface load
	// Parameter values: [Node1, Node2, Tc1, Tc2, ΔT1, ΔT2]
	surfaceLoad2.Temperature(undefined, lc, [8], "LINEAR_IN_X", [77, 78, 100, 10, 120, 20]);
	...
	surfaceLoad2.AxialStrain(undefined, lc, [9], "LINEAR", [85, 79, 80, 0.01, 0.1]);
	...
	solidLoad.Temperature(undefined, lc, [1], "LINEAR_IN_X", [91, 92, 100, 150]);
	// Strain linear in Z solid load
	// Parameter values: [Node1, Node2, ε1x, ε1y, ε1z, ε2x, ε2y, ε2z]
	solidLoad.Strain(undefined, lc, [1], "LINEAR_IN_Z", [104, 108, 0.2]);

	// Buoyancy solid load set via parameters with air density defined by attitude
	var solidLoad2 = new SolidLoad(undefined, lc, [1], "Load set via parameters", { "load_type": solid_loads.LOAD_TYPE_BUOYANCY, "uniform_magnitude": 1500 });

	/***************************************** Opening loads ****************************************/
	var openingLoad = new OpeningLoad();
	// Force uniformer/trapezoidal opening load
	openingLoad.Force(undefined, lc, [1], "UNIFORM_TRAPEZOIDAL", [1500]);
	...
	openingLoad.Force(undefined, lc, [3], "LINEAR_TRAPEZOIDAL", [97, 66, 92, 500, 1000, 1500]);
	// Force linear /trapezoidal opening load with other direction
	openingLoad.Force(undefined, lc, [2], "LINEAR_TRAPEZOIDAL", [97, 66, 92, 500, 1000, 1500], "GLOBAL_X_OR_USER_DEFINED_U_TRUE");
}
