...
for (var i = 0; i < 5; ++i) {
    new LoadCase().StaticAnalysis(undefined, "Static analysis", static_analysis_settings[1].no, "IMPOSED_LOADS_CATEGORY_A_DOMESTIC_RESIDENTIAL_AREAS_QI_A");
}
...
resultCombination2.EnvelopePermanent(undefined, design_situations[1].no, [[load_cases[5]], [load_cases[8]]]);
resultCombination2.SRSSCombination(true, "SIGN_ACCORDING_TO_LC_CO", load_cases[8]);
var resultCombination3 = new ResultCombination();
resultCombination3.EnvelopeTransient(undefined, design_situations[1].no, [[load_cases[5]], [load_cases[8]]]);
//resultCombination3.GenerateSubCombinations(); // Check box is always disabled?
var resultCombination4 = new ResultCombination();
resultCombination4.EnvelopeTransient(undefined, design_situations[1].no, [[load_cases[5]], [load_cases[8]]]);
